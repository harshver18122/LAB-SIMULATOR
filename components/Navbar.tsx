    ...
    <header className="sticky top-0 z-40 bg-white/95 backdrop-blur-md border-b border-slate-200 transition-colors">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between gap-4">
        ...
        <Link href="/" className="flex items-center gap-2.5 group">
          <div className="w-9 h-9 rounded-xl bg-[#0F2942] flex items-center justify-center text-white shadow-xs group-hover:bg-[#2563EB] transition-colors">
          ...
          <div className="flex flex-col">
            <span className="font-bold text-base text-[#0F2942] tracking-tight flex items-center gap-1.5 leading-tight">
              AI Lab Simulator
              <span className="text-[10px] bg-[#2563EB]/10 text-[#2563EB] font-semibold px-2 py-0.5 rounded-full border border-[#2563EB]/20">ACADEMY</span>
            </span>
            <span className="text-[10px] text-slate-500 font-medium">Virtual STEM Laboratory</span>
          </div>
        </Link>
            ...
            return (
              <Link
                ...
                className={`px-3.5 py-2 rounded-lg text-xs font-semibold transition-all ${
                  isActive
                    ? 'bg-slate-100 text-[#0F2942] font-bold'
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className="p-2 rounded-lg text-slate-600 hover:bg-slate-100 transition-colors flex items-center gap-2 border border-slate-200 bg-slate-50/60"
            title="Search (Ctrl+K)"
          >
            <Search className="w-4 h-4 text-slate-400" />
            <span className="text-[11px] text-slate-400 font-mono hidden lg:inline px-1 bg-white rounded border border-slate-200">⌘K</span>
          </button>
          ...
          <div className="relative">
            <button
              ...
              className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg border border-slate-200 bg-slate-50 text-[#0F2942] font-semibold text-xs hover:bg-slate-100 transition-colors"
            >
            ...
            {roleDropdownOpen && (
              <div className="absolute right-0 mt-2 w-48 bg-white rounded-xl shadow-xl border border-slate-200 py-1.5 z-50 animate-in fade-in slide-in-from-top-2">
                <div className="px-3 py-1 text-[10px] font-bold tracking-wider text-slate-400 uppercase">Switch Role View</div>
                {(['student', 'teacher', 'admin'] as UserRole[]).map((r) => (
                  <button
                    ...
                    className={`w-full text-left px-3 py-2 text-xs font-medium flex items-center justify-between hover:bg-slate-50 transition-colors ${
                      user.role === r ? 'text-[#2563EB] font-bold bg-blue-50/50' : 'text-slate-700'
                    }`}
                  >
                    ...
                    notifications.map((n) => (
                      <div
                        ...
                        className={`p-2.5 rounded-lg text-xs cursor-pointer transition-colors border ${
                          n.read ? 'bg-slate-50/50 border-slate-100 text-slate-500' : 'bg-blue-50/40 border-blue-100 text-slate-800'
                        }`}
                      >
                        <div className="font-semibold flex items-center justify-between mb-0.5 text-xs text-[#0F2942]">
                          {n.title}
          ...
          {isGuest ? (
            <div className="hidden sm:flex items-center gap-2 ml-1">
              <button
                ...
                className="px-3.5 py-2 rounded-lg text-xs font-semibold text-[#0F2942] hover:bg-slate-100 transition-colors"
              >
                Login
              </button>
              <button
                onClick={() => openAuthModal('register')}
                className="px-4 py-2 rounded-lg text-xs font-semibold text-white bg-[#2563EB] hover:bg-[#1D4ED8] shadow-xs transition-all"
              >
                Get Started
              </button>
            </div>
              ...
              {userDropdownOpen && (
                <div className="absolute right-0 mt-2 w-56 bg-white rounded-xl shadow-2xl border border-slate-200 py-2 z-50">
                  <div className="px-4 py-2 border-b border-slate-100">
                    ...
                    <span className="inline-block mt-1 text-[10px] font-semibold uppercase bg-blue-50 text-[#2563EB] px-2 py-0.5 rounded">
                      {user.role} Account
                    </span>
                  </div>
      ...
      {mobileMenuOpen && (
        <div className="md:hidden border-t border-slate-200 bg-white px-4 pt-3 pb-6 space-y-2 shadow-xl">
          ...
          <Link
            href={getDashboardHref()}
            onClick={() => setMobileMenuOpen(false)}
            className="block px-4 py-2.5 rounded-lg text-sm font-semibold text-[#0F2942] hover:bg-slate-100"
          >
            Dashboard
          </Link>
          {isGuest && (
            <div className="pt-4 border-t border-slate-100 flex items-center gap-2">
              <button
                ...
                className="flex-1 py-2.5 rounded-lg text-xs font-semibold text-[#0F2942] border border-slate-200"
              >
                Login
              </button>
              <button
                onClick={() => { openAuthModal('register'); setMobileMenuOpen(false); }}
                className="flex-1 py-2.5 rounded-lg text-xs font-semibold text-white bg-[#2563EB]"
              >
                Get Started
              </button>
            </div>
